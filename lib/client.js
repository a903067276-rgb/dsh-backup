  ...
  factory: (require) => {
    ...
    if (typeof document !== "undefined" && !document.getElementById("dsh-backup-style")) {
      ...
      tag.textContent = [
        ...
        ".bk-card .dir-open{background:none;border:none;padding:0;margin:0;font:inherit;color:var(--dsw-alias-state-business-primary,#4a7dff);text-decoration:underline;cursor:pointer;}",
        ".bk-card .dir-open:hover{opacity:.8;}",
        ".bk-card .list-row .icon-btn{display:inline-flex;align-items:center;background:none;border:none;padding:2px;margin:0;color:var(--dsw-alias-label-tertiary,#999);cursor:pointer;border-radius:4px;flex-shrink:0;}",
        ".bk-card .list-row .icon-btn:hover{color:var(--dsw-alias-state-business-primary,#4a7dff);background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,0.06));}",
        ".bk-card .restore{margin:8px 0 0;font-size:12px;color:var(--dsw-alias-label-secondary,#666);}",
        ".bk-card .restore ol{margin:4px 0 0;padding-left:18px;}",
        ".bk-card .restore li{margin:2px 0;}",
        ".bk-card .restore code{font-family:monospace;font-size:11px;color:var(--dsw-alias-label-primary,#333);}",
      ].join("\n");
    ...
    // 文件夹图标（线性 SVG，stroke=currentColor，14px；UI 规范：界面不用 emoji）
    function FolderIcon() {
      return react.createElement("svg", {
        width: 14, height: 14, viewBox: "0 0 24 24", fill: "none",
        stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round",
        style: { flexShrink: 0, verticalAlign: "-2px" },
      }, react.createElement("path", { d: "M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z" }));
    }
    ...
    function injectCardCss() {
      ...
      if (document.getElementById("dsh-backup-settings-card-style") !== null) return;
      const tag = document.createElement("style");
      tag.id = "dsh-backup-settings-card-style";
    ...
    function BackupSettingsPage(props) {
      ...
      // 系统打开（2026-08-21）：备份目录 → Finder 打开目录窗口；备份文件 → Finder 定位
      const openDir = async () => {
        if (!cfg) return;
        setMsg(null);
        try {
          await api("/open", "POST", { dir: cfg.backupDir });
        } catch (error) {
          setMsg("打开目录失败：" + (error instanceof Error ? error.message : String(error)));
          setMsgOk(false);
        }
      };
      const revealFile = async (name) => {
        setMsg(null);
        try {
          await api("/open", "POST", { file: name });
        } catch (error) {
          setMsg("定位失败：" + (error instanceof Error ? error.message : String(error)));
          setMsgOk(false);
        }
      };
      ...
      return react.createElement("div", { className: "bk-card" },
        // 卡片壳内不重复标题
        props && props.inCard ? null : react.createElement("h3", null, "备份"),
        ...
        status && status.lastError
          ...
          : null,
        react.createElement("div", { className: "row" }, "备份目录：", cfg
          ? react.createElement("button", {
              type: "button",
              className: "dir-open",
              title: "在 Finder 中打开备份目录",
              onClick: openDir,
            }, displayPath(cfg.backupDir, homeDir))
          : "--"),
        react.createElement("button", {
          type: "button",
        ...
        // 恢复说明（插件只备份、不自动恢复：恢复 = 手动解压覆盖，2026-08-21 用户要求文字写明）
        react.createElement("div", { className: "sep" }),
        react.createElement("div", { className: "restore" },
          react.createElement("div", { style: { fontWeight: 600, color: "var(--dsw-alias-label-primary)" } },
            "怎么恢复（插件只备份，恢复需手动操作）"),
          react.createElement("ol", null,
            react.createElement("li", null, "点上方「备份目录」，在 Finder 里打开备份文件夹，找到要恢复的那份 ",
              react.createElement("code", null, "dsh-backup-日期-时间.zip")),
            react.createElement("li", null, "双击解压（macOS 自带「归档实用工具」）"),
            react.createElement("li", null, "把解压出来的内容覆盖回原位："),
            react.createElement("li", null,
              react.createElement("div", null, react.createElement("code", null, "sessions/"), " → ", react.createElement("code", null, "~/.dsh/sessions"), "（会话记录）"),
              react.createElement("div", null, react.createElement("code", null, "profiles/"), " → ", react.createElement("code", null, "~/.dsh/profiles"), "（配置；node_modules 不备份，装好插件后 pnpm update 重装）"),
              react.createElement("div", null, react.createElement("code", null, "AGENTS.md"), " → ", react.createElement("code", null, "~/.dsh/AGENTS.md"), "（全局约定）"),
              react.createElement("div", null, "自定义备份目录：解压出的文件夹覆盖回它原来的位置")
            ),
            react.createElement("li", null, "覆盖完成后重启 dsh web 生效")
          )
        ),
    ...
    function apply(ctx) {
      ...
      // ── 插件区（0.1.7 官方插件管理页）──
      // 已装成「包」的插件在插件区是两级导航：包详情 → 组件行 → Configure 打开行详情；
      // 其配置位是 keyed 槽 `plugins.row.config`，键 = `${包名}#${条目id}`
      // （管理页内部 rowConfigKey 的拼法；todo-guard 影子实测确认，2026-09-24）。
      const ROW_CONFIG_KEY = "dsh-backup#backup";
      slots.inject("plugins.row.config", () => slots.register(
        { name: "plugins.row.config", key: ROW_CONFIG_KEY, order: 30, label: "备份" },
        () => react.createElement(BackupSettingsPage, { inCard: true }),
      ));

      // ── 包详情页（点插件进去的第一级）也直接显示设置卡 ──
      // 该级渲染的是 keyed 槽 `plugins.bundle.config`，键 = **包名**
      // （管理页：renderSlot("plugins.bundle.config", {view:"page"}, {entryKey: pkg.name})，
      //  只有 ledger.bundles 里有这个键，包详情页才会出现配置区 data-plugin-config）。
      slots.inject("plugins.bundle.config", () => slots.register(
        { name: "plugins.bundle.config", key: "dsh-backup", order: 30, label: "备份" },
        () => react.createElement(BackupSettingsPage, { inCard: true }),
      ));
      // 插件卡片（设置双入口统一口径 2026-08-21：有配置项的插件 = 侧边栏页 + 插件卡片）
      injectCardCss();
      slots.inject("settings.plugin.item", () => slots.register(
        { name: "settings.plugin.item", key: "backup", id: "backup" },
        () => react.createElement(SettingsCardShell, { title: "备份", desc: "会话与配置备份：周期、轮换、自定义目录" },
          react.createElement(BackupSettingsPage, { inCard: true }))
      ));
