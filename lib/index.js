 ...
 *   POST /api/dsh-backup/open    系统打开 { dir 配置目录 | file 备份文件名 }（Finder/资源管理器）
 *   POST /api/dsh-backup/config  保存配置（写 profile 条目 config；0.1.7 起 volatile 字段即时生效）
 */
...
// 字段全部可选；缺失时仍由代码里的默认值兜底（默认值口径与 0.1.x 完全一致）。
export const Config = z.object({
  backupDir: z.string().volatile(),
  customDirs: z.array(z.string()).volatile(),
  intervalHours: z.number().volatile(),
  keepCount: z.number().volatile(),
...
export function apply(ctx, rawConfig) {
  ...
  // ── 生效配置（实时视图）──
  // 0.1.7 起不再是"拍平成快照的 state"：每次属性访问都从 volatile 引用现读
  // → 设置页保存即生效，不需要重启、也不需要重挂插件。读点写法（state.xxx）保持不变。
  // 字段缺失/类型不对 → 回退默认值（与老代码同一套判断，行为不变）。
  const state = {
    get backupDir() {
      const v = refValue(cfgIn.backupDir)
      return typeof v === 'string' && v !== '' ? v : DEFAULT_BACKUP_DIR
    },
    get customDirs() {
      const v = refValue(cfgIn.customDirs)
      return Array.isArray(v) ? v.filter((s) => typeof s === 'string' && s !== '') : []
    },
    get intervalHours() {
      const v = refValue(cfgIn.intervalHours)
      return typeof v === 'number' && v > 0 ? v : 6
    },
    get keepCount() {
      const v = refValue(cfgIn.keepCount)
      return typeof v === 'number' && v > 0 ? v : 10
    },
    get enabled() {
      const v = refValue(cfgIn.enabled)
      return v !== false
    },
  }

  /** 当前生效配置快照（HTTP 响应用：写完配置后回的就是"真读到的值"，不是请求体回显）。 */
  const liveConfig = () => ({
  ...
  /**
   * 定时调度：一条"链式 timeout"守望（每轮跑完再挂下一轮），返回 disposer（卸载时清理）。
   * 0.1.7 起配置可热改 → 不用固定 interval（固定周期下改了间隔也得等下一拍才换）：
   * 每轮重新挂之前**现读** enabled / intervalHours，设置页改完下一轮即按新值走。
   * 安全行为不变：enabled=false 时**一个定时都不挂**（nextAt=null），即"关着就什么都不排"；
   * 已挂的那一轮到点也会再确认一次开关，等待期间被关掉就不跑（手动"立即备份"不受影响）。
   * 关掉后链子自然停；设置页再打开 enabled 时由 rearmIfIdle() 重新点火（免重启）。
   */
  const scheduler = (() => {
    const pending = []
    let stopped = false
    ...
    const arm = () => {
      ...
      pending.push(timer.timeout(() => {
        armed = false
        if (state.enabled) runBackup().catch(() => { /* 错误已进 runtime.lastError */ })
        arm()
      }, ms))
    }
    return {
      ...
      dispose() {
        ...
        for (const dispose of pending) {
          try { dispose() } catch (e) { /* 已触发过的 disposer 重复调用无副作用 */ }
        }
      },
    }
  })()
  ...
  }, 'dsh-backup: schedule timer')

  /** enabled 从关改到开时链子已经停了 → 重新点火（不然要重启才恢复定时）。 */
  function rearmIfIdle() {
    if (state.enabled && !scheduler.armed) scheduler.start()
  }
   ...
   *   保存完立即生效（不重挂插件、不重启），所以不再需要"写文件 + 等重启"。
   *   只有老宿主没有 settings.update 时才回落"手写 patch 文件"的老路径（那条路径确实要重启）。
   */
  ...
  /** 同源校验（防本地 CSRF）：Origin 或 Host 必须为本机（fm/perm-guard 同款先例）。 */
  ...
  /** realpath 规范化（目录不存在时回退 resolve 绝对路径），用于配置目录精确比对。 */
  function normalizeDir(p) {
    try { return realpathSync(p) } catch { return resolve(p) }
  }

  /**
   * 系统打开路径（execFile 直接传参，不经 shell，路径安全；样板：dsh-file-mentions）：
   *   reveal=false：macOS open（打开目录窗口）/ Windows explorer / Linux xdg-open
   *   reveal=true：macOS open -R（Finder 定位选中）/ Windows explorer /select, / Linux xdg-open
        ...
        try {
          ...
          if (pathname === '/api/dsh-backup/status') {
            writeJson(res, 200, {
              ...
              // 关掉 enabled 时不再报"下次备份时间"（已挂的那一轮到点也会跳过），避免显示骗人
              nextAt: state.enabled ? runtime.nextAt : null,
            })
            return
          }
          ...
          if (pathname === '/api/dsh-backup/open' && req.method === 'POST') {
            // 系统打开：只接受"配置里声明过的目录"（realpath 精确匹配）或备份列表里的 zip 文件名，
            // 不接受任意路径——配置目录 = 用户授权（同 perm-guard trustedDirs 哲学）。
            if (!isSameOrigin(req)) {
            ...
            const dir = body && typeof body.dir === 'string' && body.dir !== '' ? body.dir : null
            let target = null
            if (file !== null) {
              // 备份列表行：Finder 定位该 zip（命名规则校验，防目录穿越）
              if (!/^dsh-backup-\d{8}-\d{6}\.zip$/.test(file)) {
                writeJson(res, 400, { ok: false, error: '非法文件名' })
                return
              }
              const abs = join(state.backupDir, file)
              if (!existsSync(abs)) {
                writeJson(res, 404, { ok: false, error: '备份文件不存在' })
                return
              }
              target = { abs, isDir: false, reveal: true }
            } else if (dir !== null) {
              const allowed = [state.backupDir, ...state.customDirs]
              const want = normalizeDir(dir)
              if (!allowed.some((a) => normalizeDir(a) === want)) {
                writeJson(res, 403, { ok: false, error: '只允许打开配置中声明的目录' })
                return
              }
              target = { abs: dir, isDir: true, reveal: false }
            } else {
              writeJson(res, 400, { ok: false, error: '缺少 dir 或 file' })
              return
            }
            const opened = await systemOpen(target.abs, target.isDir, target.reveal)
            writeJson(res, opened ? 200 : 500, opened
              ? { ok: true }
              : { ok: false, error: '系统打开失败（平台: ' + process.platform + '）' })
            return
          }
          ...
          if (pathname === '/api/dsh-backup/config' && req.method === 'POST') {
            ...
            try {
              ...
              // 回"写完后真读到的值"（不是请求体回显）：条目 id 对不上/没落上时一眼能看出
              writeJson(res, 200, { ok: true, message: '已生效（无需重启）', config: liveConfig() })
            } catch (error) {
