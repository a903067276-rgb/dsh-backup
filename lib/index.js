 ...
 *   打包为 .tgz（纯 node 实现 tar+gzip，见 lib/tar.js——不用 shell 跑 tar，
...
// 硬依赖：webServer（HTTP 路由）/ timer（定时）。备份用纯 node API（lib/tar.js），不依赖 shell：
...
export function apply(ctx, config) {
  ...
  /** profiles 下 node_modules 排除前缀（tar 打包器按 archive 路径前缀匹配）。 */
  ...
  function listBackups() {
    try {
      return readdirSync(state.backupDir)
        .filter((n) => n.startsWith(BACKUP_PREFIX) && n.endsWith('.tgz'))
        .sort()
        .map((n) => {
  ...
  function pruneBackups() {
    let names = []
    try {
      names = readdirSync(state.backupDir).filter((n) => n.startsWith(BACKUP_PREFIX) && n.endsWith('.tgz')).sort()
    } catch {
      return
    }
  ...
  async function runBackup() {
    ...
    try {
      ...
      const out = join(state.backupDir, BACKUP_PREFIX + stamp() + '.tgz')
      const warnings = await createTgz(out, pairs, excludePatterns())
      ...
      handler: async (req, res) => {
        try {
          ...
          if (pathname === '/api/dsh-backup/delete' && req.method === 'POST') {
            ...
            // 防目录穿越：只允许删本插件命名规则的文件（dsh-backup-YYYYMMDD-HHmmss.tgz）
            if (!/^dsh-backup-\d{8}-\d{6}\.tgz$/.test(name)) {
